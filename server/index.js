...
app.post('/api/auth/change-password', auth, (req, res) => {
  ...
  if(uIndex === -1) return res.status(404).json({ success:false, message:'User not found' })
  if(users[uIndex].password !== currentPassword) return res.status(400).json({ success:false, message:'Password saat ini salah' })
...
app.post('/api/borrowings', auth, (req, res) => {
  ...
  const item = items.find(i => i.id === +itemId)
  if (!item) return res.status(404).json({ error: 'Item not found' })
  const id = borrowings.length ? Math.max(...borrowings.map(x=>x.id)) + 1 : 1
  const b = {
    id,
    itemId: +itemId,
    itemName: item.name,
...
app.patch('/api/borrowings/:id/approve', auth, isAdmin, (req, res) => {
  ...
  b.status = 'borrowed'
  b.borrowDate = new Date().toISOString().slice(0,10)
  const trans = {

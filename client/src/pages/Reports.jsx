...
function statusLabel(row) {
  if (row.status === 'pending') return 'Menunggu ACC'
...
export default function Reports() {
  ...
  const txIn = useMemo(
    () => transactions.filter(x => x.type === 'in' && inRange(x.date, inStart, inEnd)),
    [transactions, inStart, inEnd]
  )

  const txOut = useMemo(
    () => transactions.filter(x => x.type === 'out' && inRange(x.date, outStart, outEnd)),
    [transactions, outStart, outEnd]
  )

  const borFiltered = useMemo(
    () => borrowings.filter(b => inRange(b.submittedAt || b.borrowDate, borStart, borEnd)),
    [borrowings, borStart, borEnd]
  )
  ...
  const downloadStock = () => {
    ...
    autoTable(doc, {
      startY: 34,
      head: [['No', 'Kode', 'Nama', 'Kategori', 'Lokasi', 'Kondisi', 'Layanan', 'Stok', 'Min Stok', 'Status']],
      body,
      styles: { fontSize: 8.5, halign: 'left', valign: 'middle' },
  ...
  const downloadIn = () => {
    ...
    const totalMasuk = txIn.reduce((s, x) => s + (Number(x.quantity) || 0), 0)
    const totalHarga = txIn.reduce((s, x) => s + (Number(x.totalPrice) || 0), 0)
    doc.setFontSize(10)
  ...
  const downloadOut = () => {
    ...
    const y = doc.lastAutoTable?.finalY || 34
    const totalKeluar = txOut.reduce((s, x) => s + (Number(x.quantity) || 0), 0)
    doc.setFontSize(10)
  ...
  const downloadBorrow = () => {
    const doc = new jsPDF('p', 'mm', 'a4')
    makeHeader(doc, `Laporan Peminjaman / Penyewaan ${borStart} s.d ${borEnd}`)
    const body = borFiltered.map((b, idx) => [
      idx + 1,
      borrowTypeLabel(b.borrowType),
      b.itemName || '-',
      b.borrowerName || '-',
      b.borrowerPhone || '-',
      fmt(b.quantity || 0),
    ...
    autoTable(doc, {
      startY: 34,
      head: [['No', 'Jenis', 'Barang', 'Nama', 'No HP', 'Jumlah', 'Tgl Submit', 'Rencana Kembali', 'Tgl Kembali', 'Status', 'Catatan']],
      body,
      styles: { fontSize: 7.5 },
    ...
    doc.setFontSize(10)
    doc.text(
      `Total: ${borFiltered.length} | Aktif: ${aktif} | Dikembalikan: ${kembali} | Pending: ${pending} | Ditolak: ${tolak}`,
      14,
      y + 8
    )
    makeFooter(doc)
    doc.save(`Laporan_Peminjaman_Penyewaan_${borStart}_sd_${borEnd}.pdf`)
  }
  ...
  const downloadAll = () => {
    ...
    const borAll = borrowings.filter(b => inRange(b.submittedAt || b.borrowDate, allStart, allEnd))
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Total Stok', fmt(items.reduce((s, x) => s + (Number(x.stock) || 0), 0))],
        ['Transaksi Masuk (rentang)', fmt(txInAll.length)],
        ['Transaksi Keluar (rentang)', fmt(txOutAll.length)],
        ['Pengajuan Aktif (rentang)', fmt(borAll.filter(b => b.status === 'borrowed').length)],
    ...
    autoTable(doc, {
      ...
      styles: { fontSize: 8 },
      headStyles: { fillColor: [37, 99, 235], halign: 'center' },
      columnStyles: { 0: { cellWidth: 10, halign: 'right' }, 7: { halign: 'right' }, 8: { halign: 'right' } }
    })
    ...
    doc.addPage()
    makeHeader(doc, `Lampiran D: Peminjaman & Penyewaan ${allStart} s.d ${allEnd}`)
    autoTable(doc, {
      startY: 34,
      head: [['No', 'Jenis', 'Barang', 'Nama', 'No HP', 'Jumlah', 'Tgl Submit', 'Rencana Kembali', 'Tgl Kembali', 'Status', 'Catatan']],
      ...
      styles: { fontSize: 7.5 },
      headStyles: { fillColor: [234, 179, 8], halign: 'center' },
      columnStyles: { 0: { cellWidth: 10, halign: 'right' }, 5: { halign: 'right' } }
    })
  ...
  const cards = [
    {
      ...
      desc: 'Daftar lengkap semua barang dengan status stok dan layanan terkini',
    ...
    },
    {
      ...
      desc: 'Riwayat barang masuk pada rentang waktu terpilih',
      onClick: downloadIn,
      range: { start: inStart, end: inEnd, setStart: setInStart, setEnd: setInEnd }
    },
    {
      icon: TrendingDown,
      title: 'Laporan Barang Keluar',
      desc: 'Riwayat barang keluar pada rentang waktu terpilih',
      onClick: downloadOut,
      range: { start: outStart, end: outEnd, setStart: setOutStart, setEnd: setOutEnd }
    },
    {
      icon: History,
      title: 'Laporan Peminjaman & Penyewaan',
      desc: 'Data peminjaman, penyewaan, dan pengembalian pada rentang waktu terpilih',
      onClick: downloadBorrow,
      range: { start: borStart, end: borEnd, setStart: setBorStart, setEnd: setBorEnd }
    }
  ]
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            disabled={loading}
            onClick={downloadAll}
            className="btn-primary h-10 mt-5 disabled:opacity-60 disabled:cursor-not-allowed"
          >
                  ...
                  <div className="flex items-end gap-2 mb-3">
                    <div className="flex flex-col">
                      ...
                      <input
                        ...
                        value={range.start}
                        max={range.end}
                        onChange={e => range.setStart(e.target.value)}
                      />
                    </div>
                    <div className="flex flex-col">
                      <label className="text-xs text-gray-500 mb-1">Sampai</label>
                      <input
                        ...
                        value={range.end}
                        min={range.start}
                        onChange={e => range.setEnd(e.target.value)}
                      />
                    </div>
                  </div>
                )}

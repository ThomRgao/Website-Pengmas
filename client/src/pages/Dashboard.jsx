...
import { Package, AlertCircle, ShoppingCart, TrendingUp, TrendingDown, User } from 'lucide-react'
import api from '../api'
...
export default function Dashboard(){
  ...
  const stats = useMemo(()=>({
    totalItems: items.reduce((s, it)=>s+(it.stock||0),0),
    lowStock: items.filter(it=>(it.stock||0)<=(it.minStock||0)).length,
    borrowed: borrowings.filter(x=>x.status==='borrowed').length,
    itemsIn: transactions.filter(t=>t.type==='in').reduce((s,t)=>s+(t.quantity||0),0),
    itemsOut: transactions.filter(t=>t.type==='out').reduce((s,t)=>s+(t.quantity||0),0)
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="stat-card bg-gradient-to-br from-amber-500 to-amber-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-amber-100 text-xs mt-2">Perlu restock</p>
            </div>
            <div className="bg-white bg-opacity-20 p-4 rounded-xl">
              <AlertCircle size={32}/>
            </div>
        ...
        <div className="stat-card bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm font-medium">Dipinjam</p>
              <h3 className="text-3xl font-bold mt-2">{stats.borrowed}</h3>
              <p className="text-purple-100 text-xs mt-2">Aktif saat ini</p>
            </div>
            <div className="bg-white bg-opacity-20 p-4 rounded-xl">
        ...
        <div className="stat-card bg-gradient-to-br from-emerald-500 to-emerald-600 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-emerald-100 text-sm font-medium">Aktivitas Bulan Ini</p>
              <h3 className="text-2xl font-bold mt-2">+{stats.itemsIn} / -{stats.itemsOut}</h3>
              <p className="text-emerald-100 text-xs mt-2">Masuk vs Keluar</p>
            </div>
            <div className="bg-white bg-opacity-20 p-4 rounded-xl">
              <TrendingUp size={28}/>
            </div>
      ...
      {stats.lowStock>0 && (
        <div className="card bg-amber-50 border border-amber-200">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h4 className="font-bold text-amber-900 mb-2">Peringatan Stok Menipis</h4>
              <p className="text-amber-800 mb-3">Ada {stats.lowStock} barang yang stoknya sudah mencapai batas minimum. Segera lakukan restocking.</p>
              <a href="/items" className="btn-primary bg-amber-600 hover:bg-amber-700 text-sm">Lihat Detail Barang</a>
            </div>
          </div>
        </div>
      )}

...
import { Check, X, RefreshCcw, TrendingDown, ShoppingCart, Activity } from 'lucide-react'
import api from '../api'
...
export default function ItemsOut() {
  ...
  const returnPending = isAdmin
    ? borrowings.filter(b => b.status === 'borrowed' && b.returnRequestStatus === 'pending')
    : []
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {isAdmin
              ? 'Setujui pengajuan, pantau barang keluar, dan verifikasi pengembalian'
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm text-gray-600">Sedang Dipinjam / Disewa</p>
      ...
      {isAdmin && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="card overflow-hidden">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Sedang Dipinjam / Disewa</h3>
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left p-3">Nama</th>
          ...
          <div className="card overflow-hidden">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Menunggu Verifikasi Return</h3>
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left p-3">Nama</th>
              ...
              <tbody>
                ...
                {returnPending.map(b => (
                  <tr key={b.id} className="border-b border-gray-100">
                    <td className="p-3">{b.borrowerName}</td>
                    <td className="p-3">{b.itemName}</td>
                    <td className="p-3">{b.returnRequestedAt || '-'}</td>
                    <td className="p-3">
                      <button
                        onClick={() => verifyReturn(b.id)}
